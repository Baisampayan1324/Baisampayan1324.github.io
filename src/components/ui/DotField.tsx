...
const DotField = memo(({
  ...
  const fullGlowId = `dot-field-glow-${baseId}`;
    ...
    function tick() {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (distSq < crSq && eng > 0.01) {
          ...
          if (isBulge) {
            const time = 1 - dist / cr;
            const push = time * time * p.bulgeStrength * eng;
        ...
        const drawShape = (x: number, y: number, r: number) => {
          if (p.dotShape === 'square') {
